# lib_blind.py делает две вещи:
# * Слепая подпись Чаума (RSA) — blind()/unblind(): 
# 		регистратор подписывает ослеплённый хеш ключа избирателя, не видя сам ключ.
# * Сырые подписи RSA — signature()/verify(): 
# 		приватная операция m^d mod n, 
# 		публичная sig^e mod n. 
# 		Причём verify() возвращает сам хеш (RSA-симметрия: подпись «расшифровывается» публичным ключом). 
# 		Это используется для подписи бюллетеней, секретных ключей и сертификации ключей регистратора.
